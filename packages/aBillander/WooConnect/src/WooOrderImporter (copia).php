...
//        return $invoice_id;
    ...
    public function setCustomer()
    {
        ...
        $customer = Customer::where('webshop_id', $customer_webshop_id )->first();

        if ($customer) abi_r($customer->id, true);
...
//			'allow_login' => $order[''],
		...
		$bstate = \App\State::findByIsoCode( (strpos($order['billing']['state'], '-') ? '' : $order['billing']['country'].'-').$order['billing']['state'] );
		if ($bstate) {
			$state    = $bstate->name;
			$state_id = $bstate->id;
		}
...
//			'longitude' => $order[''],
		...
		$sstate = \App\State::findByIsoCode( (strpos($order['shipping']['state'], '-') ? '' : $order['shipping']['country'].'-').$order['shipping']['state'] );
		if ($sstate) {
			$state    = $sstate->name;
			$state_id = $sstate->id;
		}
